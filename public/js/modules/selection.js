...
let holdTimer = null, holdSx = 0, holdSy = 0;

function startHold(x, y, onFire) {
  holdSx = x; holdSy = y;
...
function moveHold(x, y) {
  if (holdTimer && (Math.abs(x - holdSx) > MOVE_CANCEL_PX || Math.abs(y - holdSy) > MOVE_CANCEL_PX)) {
 ...
 */
export function attachCardInteraction(el, { getId, onOpen, onToggleSelect, onModifierSelect }) {
  let clickTimer = null;
  ...
  el.addEventListener('mousedown', e => {
    ...
    startHold(e.clientX, e.clientY, () => { justLongPressed = true; onToggleSelect(getId()); });
  });
  el.addEventListener('touchstart', e => {
    if (isIgnoredTarget(e)) return;
    const t = e.touches[0];
    startHold(t.clientX, t.clientY, () => { justLongPressed = true; onToggleSelect(getId()); });
  }, { passive: true });
