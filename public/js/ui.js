...
export function openDrawer() { document.getElementById('hbr-drawer').classList.add('open'); document.getElementById('hbr-backdrop').classList.add('open'); }
export function closeDrawer() { document.getElementById('hbr-drawer').classList.remove('open'); document.getElementById('hbr-backdrop').classList.remove('open'); }
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'n' && !inp) { window.openNewTapeModal?.(); }
  const selectedIds = window._getSelectedIds?.() || new Set();
...
function updateSoundBtn(){
  ...
  if(btn)btn.textContent=getSoundEnabled()?'🔊 Sounds':'🔇 Sounds Off';
}
...
async function runHealthCheck(){
  ...
  try{
    ...
    const remeds=[];
    if(h.db!=='ok')remeds.push('• DB: Check DATABASE_URL in .env and that Neon is reachable.');
    if(h.ollama!=='ok')remeds.push('• Ollama: Ensure the ollama container is running. Pull a model with "ollama pull llava:7b".');
